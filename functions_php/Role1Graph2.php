...
$searchword_cnt=0;
	...
	$searchwords_arr[$searchword_cnt]= "Patient Complaints and Issues";
	$searchword_cnt++;	
...
{
$query ="SELECT DATE_FORMAT(DATE(published),'%Y,%c,%d') as pDate, COUNT(*) as pCount FROM listeningdata WHERE DATE( published ) > CURDATE( ) - INTERVAL 10 DAY " . $fixed_brand . $clause ." " . $querystring ." GROUP BY DATE(published) order by published"; 
}
else
{
